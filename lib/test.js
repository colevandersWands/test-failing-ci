  ...
  for (const nextPath of paths) {
    ...
    if (throwErrors && errors.length !== 0) {
      const allErrors = errors
        .map(err => cleanCallstack(err.stack))
        .join('\n\n');
      console.log(allErrors);
      const failureError = new Error(
        "testing CI: to find the failing tests go to the top file in the callstack and read that spec's report\n--- begin failing assertions ---\n\n" +
          allErrors +
          '\n\n--- end failing assertions ---\n'
      );
      core.setFailed(failureError);
    }
  }
};
